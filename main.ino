...
int16_t eeprom_get_tag_id_by_uid(uint8_t uid[UID_SIZE])
{
  ...
  Serial.print("Unknown tag. Uid:");
  Serial.println(bytes2hexstr(uid, UID_SIZE));
  return -1;
}
...
void tag_print(tag_t *tag)
{
  ...
  Serial.print(tag->id);
  Serial.print(",UID:");
  Serial.print(bytes2hexstr(tag->uid, 10));
  Serial.print(",PERM:");
  Serial.print(tag->perm);
  Serial.print(",NAME:");
  Serial.println(bytes2hexstr(tag->name, NAME_SIZE));
}
...
void setup()
{
  ...
  SPI.begin();
	card_reader.PCD_Init();
	delay(4);
	card_reader.PCD_DumpVersionToSerial();
